...
import { createSessionFilename } from "../../rehab/exporters";
...
function downloadSessionJson(createExport: () => unknown) {
  downloadTextFile({
    content: JSON.stringify(createExport(), null, 2),
    filename: createSessionFilename("json"),
    mimeType: "application/json;charset=utf-8",
  });
}

export default function CafeBaristaModule({
  onBackToModules,
}: {
  ...
  return (
    <div className="barista-app">
      <header className="barista-header module-training-header">
        <div className="module-training-title">
          <button className="icon-btn" type="button" onClick={onBackToModules}>
            <ArrowLeft size={20} />
            <span className="sr-only">返回模組</span>
          </button>
          <div>
            <p className="barista-kicker">動作訓練模組</p>
            <h1>Café Barista</h1>
          </div>
        </div>
        ...
        >
          <div className="barista-stage-hud">
            ...
            <div className="pour-meter" aria-label="倒水進度">
              <span>
                {metrics.completedCups}/{settings.cupsPerSession} 杯
              </span>
